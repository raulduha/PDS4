...
const StationUpdate = () => {
    ...
    useEffect(() => {
      axios.get(`https://backend-p3.vercel.app/stations/update/${station_id}`)
        .then(response => {
    ...
    const handleSubmit = async e => {
      ...
      try {
        const response = await axios.put(`https://backend-p3.vercel.app/stations/update/${station_id}/${formData.name}/${formData.address}/`);
